...
test.describe('New Arrivals Section', () => {
  ...
  test('Homepage → close popup → scroll to New Arrivals → verify all products', async ({ page }) => {
    ...
    const heading = page.locator('span').filter({ hasText: /^New Arrivals$/i }).first();
    await heading.waitFor({ state: 'attached', timeout: 10000 });
    await heading.scrollIntoViewIfNeeded();
    await page.waitForTimeout(1500);
    console.log('✅ Step 3: Scrolled to New Arrivals section');

    const grid = page.locator('[class*="glider-track"][class*="grid-rows-2"]').nth(NEW_ARRIVALS_INDEX);
    ...
    console.log('\n\n' + '═'.repeat(85));
    console.log('  NEW ARRIVALS — SUMMARY');
