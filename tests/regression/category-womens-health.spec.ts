...
test.describe("Category — Women's Health", () => {

  test("Open Women's Health category → verify heading, product cards, names, prices, links", async ({ page }) => {
    ...
    expect(h1Text).toMatch(/women.*health/i);
    ...
    // Step 6: Verify each card has a product name (H2) and price (₹)
    ...
    // Step 7: Verify product links point to staging
    const allLinksValid = cardData.every(c => c.hasLink);
    expect(allLinksValid, 'All product cards should have a valid link').toBe(true);
    console.log("✅ Step 7: All product links are present");

    console.log("\n🎉 Women's Health category validated successfully!\n");
  });
