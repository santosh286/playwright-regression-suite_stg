...
async function openProductPDP(page: any, productId: string): Promise<string> {
  ...
  const name = (await page.locator('h1').first().textContent({ timeout: 10000 }))?.trim() || 'N/A';
  console.log(`📋 PDP Product Name: "${name}"`);
  return name;
}

async function capturePDPPriceBeforeCoupon(page: any): Promise<string> {
  const priceEl = page.locator('p.mt-\\[2px\\].text-\\[20px\\].font-\\[700\\]').first();
...
async function applyBestPriceCouponOnPDP(page: any): Promise<{ priceAfter: string; navigatedToCheckout: boolean }> {
  const offersHeading = page.getByRole('heading', { name: /offers for you/i });
  ...
  const onlineCoupon = page.getByText(/online payment coupon/i).first();
  ...
  const appliedCouponCard = page.locator('.appliedCoupon_couponsCard__DBs17');
  const modalVisible = await appliedCouponCard.isVisible({ timeout: 3000 }).catch(() => false);
  if (modalVisible) { await appliedCouponCard.click(); await page.waitForTimeout(1000); }
  const priceEl = page.locator('p.mt-\\[2px\\].text-\\[20px\\].font-\\[700\\]').first();
  const text = await priceEl.textContent({ timeout: 5000 }).catch(() => 'N/A');
  const price = text?.trim() || 'N/A';
...
async function markPaymentSuccess(page: any) {
  ...
  if (isUPIPage) {
    ...
    await expect(page.getByRole('button', { name: 'Submit' })).toBeEnabled({ timeout: 5000 });
...
test.describe('Place Order – Best Price Online Flow', () => {
  ...
  test('User should place order using Best Price Online coupon', async ({ page }) => {
    ...
    const priceBeforeCoupon = await capturePDPPriceBeforeCoupon(page);

    const { priceAfter: priceAfterCoupon, navigatedToCheckout } = await applyBestPriceCouponOnPDP(page);
